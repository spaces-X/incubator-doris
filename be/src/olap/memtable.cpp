...
MemTable::MemTable(int64_t tablet_id, Schema* schema, const TabletSchema* tablet_schema,
    ...
    if (support_vec){
        ...
        _block_aggregator = std::make_unique<BlockAggregator>(_schema, true);
    }else{
...
bool MemTable::insert(const vectorized::Block* block, size_t row_pos, size_t num_rows) {
    if (_is_first_insertion) {
        ...
        _block = make_shared<MutableBlock>(&cloneBlock);
...
size_t MemTable::bytes_allocated() const {
    ...
    return _block_bytes_usage + _block_aggregator->allocated_bytes();
}
...
void MemTable::_agg(const bool finalize) {
    ...
    _block_aggregator->append_block(&_sorted_block->to_block());
...
void MemTable::_sort(const bool finalize) {
    ...
    for (size_t i = 0; i < _block->rows(); i++) {
...
void MemTable::_append_sorted_block(MutableBlock* src, MutableBlock* dst) {
    ...
    for (size_t i = 0; i < row_num; i++) {
        _sorted_index_in_block.push_back(_index_for_sort[i]);
    }
